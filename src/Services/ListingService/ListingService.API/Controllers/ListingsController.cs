    ...
    public class ListingsController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateListing([FromBody] CreateListingCommand command, CancellationToken cancellationToken)
        {
            ...
            try
            {
                // In a real app, command.SellerId should be set from User.Claims
                // var authenticatedUserId = User.FindFirstValue(ClaimTypes.NameIdentifier);
                // if (authenticatedUserId == null) return Unauthorized();
                // command.SellerId = authenticatedUserId; // Override or validate
        ...
        public async Task<IActionResult> UpdateListing(string id, [FromBody] UpdateListingCommand command, CancellationToken cancellationToken)
        {
            ...
            // var authenticatedUserId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            // if (command.SellerId != authenticatedUserId) return Forbid("Not authorized to update this listing.");
            // Ensure command.SellerId is set to authenticatedUserId before passing to handler, or handler does the check.
        ...
        {
            // var authenticatedUserId = User.FindFirstValue(ClaimTypes.NameIdentifier);
            // if (sellerId != authenticatedUserId) return Forbid("Not authorized to delete this listing.");
            // var command = new DeleteListingCommand { Id = id, SellerId = authenticatedUserId }; // Use authenticated ID

            if (string.IsNullOrWhiteSpace(id) || string.IsNullOrWhiteSpace(sellerId))
                 return BadRequest(new { error = "Listing ID and Seller ID must be provided for deletion." });
            var command = new DeleteListingCommand { Id = id, SellerId = sellerId }; // For now, pass sellerId
